...
export default class StockEntryRepositoryDatabase implements StockEntryRepository {
  ...
  async getByIdItem(idItem: number): Promise<StockEntry[]> {
    ...
    for (const stockEntryData of stockEntriesData) {
      stockEntries.push(new StockEntry(stockEntryData.id_item, stockEntryData.operation, stockEntryData.quantity, new Date(stockEntryData.entry_date)));
    }
    return stockEntries;
  }
  async save(stockEntry: StockEntry): Promise<void> {
    await this.connection.query("insert into ccca.stock_entry (id_item, operation, quantity, entry_date) values ($1, $2, $3, $4)", [
      stockEntry.idItem,
